...
//   const products = Object.keys(productData);
  ...
  const handleSubmit = () => {
    newProducts.forEach((product) =>{ dispatch(AddProduct(product))
  ...
  return (
    <div className="add-product-modals">
     ...
     <div>
        <><b>Product</b></>
      <select name="product" value={Product} multiple onChange={handleChange}>
        
        {products.map((product) => (
          <option key={product} value={product} className={Product===product?"select":"nonselected"}>
            {product}
          </option>
        ))}
      </select>
      ...
      <div>
        <>Material</>
      <select name="material" value={Material} onChange={handleMaterialChange} multiple disabled={!Product}>
       
        {availableMaterials.map((material) => (
          <option key={material} value={material} className={Material===material?"select":"nonselected"}>
            {material}
          </option>
        ))}
      </select>
      ...
      <div>
        <>Grades</>
      <select name="grade" multiple={true} value={Grades} onChange={handleGradeChange} disabled={!Material}>
        {availableGrades.map((grade) => (
            
          <option key={grade} value={grade} className={Grades.includes(grade) ? "selected" : "nonselected"}>
            
           
            <label for="scales">{grade}  <input type="checkbox" style={{border:"1px solid black"}} readOnly />
            </label>
